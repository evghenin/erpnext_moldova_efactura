"""Match an existing Purchase Invoice to eFactura Buyer lines and totals."""
...
def money_precision(currency: str | None = None) -> int:
	try:
		prec = frappe.get_precision("eFactura Buyer", "total")
		if prec is not None:
			return cint(prec)
	except Exception:
		pass
...
def qty_precision() -> int:
	try:
		prec = frappe.get_precision("eFactura Buyer Item", "ef_qty")
		if prec is not None:
			return cint(prec)
	except Exception:
		pass
...
def price_matches(buyer_row, pi_row, mprec: int) -> bool:
	...
	rate_ok = eq(pi_rate, buyer_row.rate, mprec) or eq(pi_rate, buyer_row.rate_with_vat, mprec)
...
def collect_totals_and_line_errors(
	...
	if not eq(buyer.total, pi.grand_total, mprec):
		errors.append(
	...
	if not eq(buyer.vat_total, pi.total_taxes_and_charges, mprec):
		errors.append(
...
def buyer_row_qty(row) -> float:
	return flt(row.ef_qty) if flt(row.ef_qty) else flt(row.qty)
...
def collect_document_errors(buyer, pi) -> list[str]:
	...
	if buyer.purchase_invoice and buyer.purchase_invoice != pi.name:
		errors.append(
			_("e-Factura is already linked to Purchase Invoice {0}").format(buyer.purchase_invoice)
		)
	other = frappe.db.get_value(
		"eFactura Buyer",
		{"purchase_invoice": pi.name, "name": ["!=", buyer.name]},
		"name",
	)
	if other:
		errors.append(
	...
	linked_on_pi = pi.get("efactura_buyer") if hasattr(pi, "get") else getattr(pi, "efactura_buyer", None)
	if linked_on_pi and linked_on_pi != buyer.name:
		errors.append(
			_("Purchase Invoice {0} is already linked to e-Factura {1}").format(pi.name, linked_on_pi)
		)
	return errors


def raise_link_error(pi_name: str, errors: list[str]):
...
def validate_and_match(buyer, pi) -> list[tuple[Any, Any]]:
	errors = collect_document_errors(buyer, pi)
	line_errors, pairs = collect_totals_and_line_errors(buyer, pi)
	errors.extend(line_errors)
	if errors:
		raise_link_error(pi.name, errors)
	return pairs
